...
const Login = () => {
  ...
  const handleSubmit = async (
    values: FormValues,
  { setSubmitting }: FormikHelpers<{ email: string; name: string; password: string; }> // Specify FormikHelpers type
  ) => {
        try {
      setSubmitting(true); 
      const response = await axios.post(`${process.env.VITE_API_BASE_URL}/login`, values);
  ...
  return (
    ...
    <div className="bg-white shadow-lg flex flex-col items-center rounded-xl overflow-hidden lg:flex-row lg:w-2/3 2xl:w-1/2">
      ...
      <p className="text-gray-500">
              Don't have an account? <Link to="/" className="text-blue-500 font-semibold underline">Sign up</Link>
            </p>
      </div>
      <div className="w-1/2">
